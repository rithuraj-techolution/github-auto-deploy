...
def js_error_check(code: str):
    ...
    try:
        ...
        if status_code != 200:
            error_message = error_to_message(result, code, "Javascript", TypescriptConfig)
    ...
    try:
        ...
        for i, changed_file in enumerate(changed_files):
            ...
            with open(changed_file, "w", encoding="utf-8") as f:
                ...
                print(f"File: {original_filename} updated successfully")
                
            title_message += f"Filname: {original_filename}\nChanges Done:\n{explanations}\n\nError Check Status on Original File : Success\n\nError Check Status on Refactored File : Success\n\nUnit Test Results : Skipped" 
            
            success_list.append({"filename": original_filename, "Error Check Status on Original File":"Success", "Error Check Status on Refactored File":"Success", "Unit Test Results":"Skipped"})
        ...
        #         file.write(code + "\n\n")
        #     # ans = long_code_refactor.rlef_api_call(refactor_branch, code, file_path, TypescriptConfig.TOKEN, code_refactor_token_limit=1500)
        #     # ans = fun(code, file_path)
        #     thread = threading.Thread(target=long_code_refactor.rlef_api_call, args=(refactor_branch, code, file_path, TypescriptConfig.TOKEN, 1500))
        #     thread.start()

        #     print("$$$$$$$$$$$$")
        print("Title Message: ", title_message)
        git_utils.git_add_commit("Refactored code updated")
        run_terminal_commands(cmd=f"git push origin {refactor_branch}")

        ans = git_utils.checkout_commit_raise_PR(source_branch, refactor_branch, username, title_message)
        if ans:
            return {"status": "Pipeline completed successfully", "Pull Request URL": ans, "Files_status": success_list}, StatusCodes.SUCCESS.value
        else:
...
def main_function(data: dict, flow: str) -> tuple:
    ...
    try:
        ...
        TypescriptConfig.set_creds(data["repo_name"], data["pat"], data["organization"])
        git_utils = GitHubUtils(config=TypescriptConfig)
        os.chdir(data["repo_name"])
        ts_project = Project(TypescriptConfig, git_utils)
        file_extensions = TypescriptConfig.FILE_EXTENSIONS
        print(f"Extensions supported in this project: {file_extensions}")
        repo_directory = os.getcwd()

        if flow == "normal":
            refactored_code_list, test_case_list, changed_files, readme_status = ts_project.refactor_unittest_pipeline(data, file_extensions=file_extensions, assistants=TypescriptAssistants, repo_directory= repo_directory)
            ...
            updated_code_list, changed_files = ts_project.feedback_refactor_pipeline(
